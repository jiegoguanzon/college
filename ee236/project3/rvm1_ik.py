...
def inverse_kinematics(p_x, p_y, p_z):
    ...
    if (np.rad2deg(q2) < -30 or np.rad2deg(q2) > 100 or np.rad2deg(q3) > 0 or np.rad2deg(q3) < -110 or np.rad2deg(q4) > 90 or np.rad2deg(q4) < -90):
        joint_variables = -1
    else:
...
def gripper_orientation(circle, nearest_circle):
    ...
    #print(gripper_cost_fcn)
    #print(gripper_cost_fcn_der)
    ...
    #print(np.rad2deg(theta_gd[epoch - 1]))
    #print(np.rad2deg(cost_gd[epoch - 1]))
